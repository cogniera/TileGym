...
Path looks like `<repo>/.claude/skills/cutile-python/`. The enclosing repo **is** TileGym. No clone needed — use it directly:

```
<repo>/src/tilegym/ops/cutile/
```

### Case 2 — skill installed elsewhere (e.g. `~/.claude/skills/`)

Path looks like `~/.claude/skills/cutile-python/`, or the skill is inside some other repo that does not ship `src/tilegym/`. TileGym is not adjacent; clone it once on first use to the cache directory and use it from there:
